...
```js
const nums = [5, 2, 9, 1];
nums.sort((a, b) => a - b);  // ছোট → বড় (comparator জরুরি!)
...
**📝 অনুশীলন**
- comparator বদলে বড় → ছোট sort করো।
...
function bubbleSort(arr) {
  ...
  for (let i = 0; i < n - 1; i++) {
    ...
    if (!swapped) break;                     // কোনো swap নেই → sorted, থামি
  }
  return arr;
}
...
**⏱️ Complexity** — worst/avg O(n²); best O(n); space O(1); stable।
...
- প্রতি pass-এ মাত্র একটা swap → swap ব্যয়বহুল হলে ভালো।

**⏱️ Complexity** — সব ক্ষেত্রে O(n²); space O(1); unstable।
...
- প্রায়-sorted হলে while প্রায় চলে না → O(n)।
...
function mergeSort(arr) {
  if (arr.length <= 1) return arr;                 // base: একটা → sorted
...
**⏱️ Complexity** — সব ক্ষেত্রে O(n log n); space O(n); stable।
...
**⏱️ Complexity** — avg/best O(n log n); worst O(n²); space O(log n); unstable।
...
**⏱️ Complexity** — সব ক্ষেত্রে O(n log n); space O(1); unstable।
...
**📝 অনুশীলন**
- min-heap দিয়ে বড় → ছোট sort করতে কী বদলাতে হবে ভাবো।
