...
বাস্তব উপমা: দুই আয়নার মাঝে দাঁড়ালে ছোট থেকে ছোট প্রতিফলন; অথবা রুশ পুতুল (matryoshka) — ভেতরে ভেতরে ছোট পুতুল।
...
**🔍 ব্রেকডাউন**
- `if (n === 0) return` → base case; না থাকলে অসীম বার ডাকবে (stack overflow)।
- `countdown(n - 1)` → প্রতিবার সমস্যা এক ধাপ ছোট।
- প্রতিটা recursion-এ সমস্যা base case-এর দিকে এগোতেই হবে।

**⏱️ Complexity** — এখানে O(n)।
...
- base case → 1 return → পিছাতে পিছাতে গুণ হয়।
- খুব বেশি গভীর recursion → **stack overflow** (মেমরি শেষ)।

**⏱️ Complexity** — time O(n), space O(n) (stack গভীরতা)।
...
**⏱️ Complexity** — sum/reverse O(n); naive fib O(2ⁿ)।
...
**নিয়ম:** সমস্যাটা স্বাভাবিকভাবে "ছোট একই সমস্যায়" ভাঙে (tree, divide-and-conquer) → recursion; সরল রৈখিক → loop।
...
**🔍 ব্রেকডাউন**
- `cache.has(n)` → আগে হিসাব করা থাকলে তৎক্ষণাৎ return।
- প্রতিটা `n` একবারই হিসাব হয় → **O(2ⁿ) থেকে O(n)**-এ নেমে আসে!
...
Recursion = base case + ছোট সমস্যায় নিজেকে ডাকা; call stack-এর উপর চলে। ক্লাসিক: factorial, sum, reverse, fibonacci। Iteration কম memory নেয়; recursion tree/graph-এ পরিষ্কার। Memoization দিয়ে O(2ⁿ) → O(n) — DP-র সূচনা।
