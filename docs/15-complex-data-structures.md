...
**⏱️ Complexity** — search/insert/delete O(log n) নিশ্চিত।
...
> - **ধাপ:** node overflow → split animation; height সমানভাবে বাড়া।
...
- **সব leaf একটা linked list-এ জোড়া** → range query (`WHERE age BETWEEN 20 AND 30`) খুব দ্রুত (এক leaf থেকে পরের leaf-এ হাঁটা)

```
B+ Tree:
            [30 | 60]              ← internal (শুধু guide)
           /    |    \
     [10 20]→[30 40 50]→[60 70]    ← leaves (data) linked
```
...
**🔍 ব্রেকডাউন**
- বড় fan-out (অনেক সন্তান) → লাখো row-এও height মাত্র ৩-৪ → ৩-৪ disk read-এ যেকোনো row।
- B+ Tree-এর linked leaf → sorted scan ও range query সহজ; এজন্য MySQL (InnoDB), PostgreSQL সবই B+ Tree index ব্যবহার করে।

**⏱️ Complexity** — search/insert/delete O(log n); disk-access-এ বিশাল সুবিধা।
...
**💡 ধারণা**
সাধারণ sorted linked list-এ search O(n) (index নেই)। **Skip List** কয়েকটা অতিরিক্ত স্তর (express lane) যোগ করে — উপরের স্তরে কম node (কিছু "লাফিয়ে" যায়), নিচের স্তরে সব node। খোঁজার সময় উপরে দ্রুত এগিয়ে দরকারমতো নিচে নামা → গড়ে **O(log n)**। randomness দিয়ে স্তর ঠিক হয় (তাই সহজ, balanced tree-র বিকল্প)।
...
- নতুন node-এর স্তর সাধারণত coin-flip (random) দিয়ে ঠিক → গড়ে ভারসাম্য।
- Redis-এর sorted set (ZSET) skip list ব্যবহার করে।

**⏱️ Complexity** — গড় search/insert/delete O(log n); space O(n)।
...
- বেশি insert হলে overflow বড় হয়ে performance পড়ে যায় → periodic **reorganization** লাগে

**💻 (ধারণা)**
```text
Index:   [A→page1] [F→page2] [M→page3]   (স্থির, sorted key → page)
Data:    page1: A,B,C  |  page2: F,G  |  page3: M,N,O  |  overflow: (নতুনগুলো)
খোঁজা "G": index-এ F-page পাই → page2 sequential scan → G।
```
...
**⏱️ Complexity** — ভালো অবস্থায় O(log n)-এর কাছাকাছি; overflow বাড়লে খারাপ।

> **🎬 Animation Spec: Static Index + Overflow**
> - **দৃশ্য:** স্থির index টেবিল + sequential data page + overflow area।
...
> - **লক্ষ্য:** static vs dynamic index-এর trade-off (B+ Tree-র প্রেরণা)।

**📝 অনুশীলন**
- ISAM vs B+ Tree: read-heavy বনাম write-heavy-তে কোনটা ভালো, ব্যাখ্যা করো।
...
**💡 ধারণা**
**Index** = ডেটার উপর একটা অতিরিক্ত কাঠামো যা খোঁজা দ্রুত করে (বইয়ের পেছনের index-এর মতো)। মূল দুই ঘরানা:

**১) Linear / Hash-based Indexing**
- key → সরাসরি জায়গা (hash table-এর মতো)
...
- Tree (B+): সামান্য ধীর equality, কিন্তু range/sort/prefix — সবই ভালো → বেশি বহুমুখী।
- বাস্তবে DBA কোন column-এ কোন ধরনের index দেবে, তা query-প্যাটার্ন দেখে ঠিক করে।

**⏱️ Complexity** — Hash: equality O(1); Tree: O(log n) কিন্তু range-সক্ষম।

> **🎬 Animation Spec: Query → Index Choice**
...
**পরবর্তী:** [16 — Problem Solving Techniques](16-problem-solving-techniques.md) →
