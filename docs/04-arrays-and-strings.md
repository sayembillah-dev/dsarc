...
মেমরিতে array-এর উপাদানগুলো **পাশাপাশি** থাকে। তাই index দিলে কম্পিউটার সরাসরি হিসাব করে ঠিক জায়গায় পৌঁছায় — এজন্য index access **O(1)**।
...
**🔍 ব্রেকডাউন**
- `arr[2]` → শুরুর ঠিকানা + (2 × প্রতি উপাদানের আকার) = সরাসরি জায়গা → O(1)।
- index সবসময় 0 থেকে শুরু, তাই শেষ index = `length - 1`।

**⏱️ Complexity** — access/update O(1)।
...
**🔍 ব্রেকডাউন**
- শুরুতে/মাঝে কিছু করলে বাকিদের **সরাতে** হয় → O(n)।
- শেষে করলে কাউকে সরাতে হয় না → O(1)।
- `splice(start, deleteCount, ...items)` → বহুমুখী: বাদ দেওয়া/যোগ করা দুটোই করে।
...
## 4.3 — Array-তে ঘোরাঘুরি (Traversal & Iteration)
...
**🔍 ব্রেকডাউন**
- একবার loop = O(n) — বেশিরভাগ সমস্যার আদর্শ লক্ষ্য।
- two-pointer অনেক সমস্যা O(n²) থেকে O(n)-এ নামিয়ে আনে (পরে বিস্তারিত)।

**⏱️ Complexity** — O(n), Space O(1)।
...
**💡 ধারণা**
2D array হলো array-এর ভেতরে array — একটা টেবিল/গ্রিড। `matrix[row][col]` দিয়ে অ্যাক্সেস।
...
**🔍 ব্রেকডাউন**
- `grid[row][col]` → আগে সারি, পরে কলাম।
- নেস্টেড loop → m×n ঘর → **O(m·n)**।
- `Array.from({length}, () => Array(cols).fill(0))` → নিরাপদভাবে খালি গ্রিড বানানোর নিয়ম (একই array শেয়ার এড়াতে)।

**⏱️ Complexity** — পুরো গ্রিড ঘুরলে O(m·n)।
...
// string ↔ array রূপান্তর (খুব দরকারি)
...
- বদলানোর কাজে: string → array → পরিবর্তন → array → string।
- `split/join` জোড়া string সমস্যায় বারবার লাগবে।

**⏱️ Complexity** — বেশিরভাগ string অপারেশন O(n) (n = দৈর্ঘ্য); concatenation-এ সাবধান (বারবার করলে O(n²) হতে পারে)।
...
Array = ধারাবাহিক index-ভিত্তিক তালিকা; access O(1), শুরুতে/মাঝে insert/delete O(n)। 2D array = গ্রিড, O(m·n)। String = immutable char array; বদলাতে split/join। Two-pointer ও single-pass প্যাটার্ন এখানেই শুরু।
