...
**⏱️ Complexity** — O(n) time; space O(h) (h = height, stack)।
...
- **সতর্কতা:** sorted ডেটা insert করলে গাছ একপাশে হেলে (linked list-এর মতো) → O(n)। সমাধান: balancing (নিচে AVL)।

**⏱️ Complexity** — গড় O(log n); worst (skewed) O(n)।
...
**🔍 ব্রেকডাউন**
- প্রতিটা node তার height মনে রাখে; balance factor = বাঁ height − ডান height।
- |bf| > 1 হলে rotation দিয়ে পুনর্বিন্যাস — মান/BST-নিয়ম অক্ষুণ্ণ রেখে।
- rotation একটা O(1) pointer-পুনর্বিন্যাস।

**⏱️ Complexity** — insert/search/delete সব O(log n) নিশ্চিত।
...
**📝 অনুশীলন**
- 1,2,3 ক্রমে insert করলে কোন rotation লাগবে ভাবো (RR → left rotation)।
...
**💡 ধারণা**
Binary tree-র প্রতিটা node-এ ১টা key, ২টা সন্তান। **B-Tree**-তে প্রতিটা node-এ **অনেক key ও অনেক সন্তান** থাকতে পারে। এতে গাছ কম গভীর হয় → disk থেকে কম বার পড়তে হয়। এজন্য database index ও file system (section 15-এ B+ Tree) এতে দাঁড়িয়ে।
...
**🔍 ব্রেকডাউন**
- অনেক key/সন্তান → কম height → disk read কম।
- "node full হলে split" — এই কৌশলে সবসময় balanced থাকে।
- বাস্তবে disk-page-এর আকারের সাথে node আকার মেলানো হয়।

**⏱️ Complexity** — search/insert/delete O(log n); কিন্তু disk-access-এ বিশাল সুবিধা।
...
**💡 ধারণা**
- **DFS** (pre/in/post) → recursion বা **stack**; গভীরে আগে।
- **BFS** (level-order) → **queue**; স্তরে স্তরে।
...
function dfsIterative(root) {
  ...
  while (stack.length) {
    const node = stack.pop();       // উপরেরটা নিই (LIFO → গভীরে)
    out.push(node.value);
...
- stack → DFS, queue → BFS — এই জোড়া মনে রাখো।
...
Tree = শ্রেণিবদ্ধ কাঠামো (root/leaf/height)। DFS: pre/in/post (recursion/stack), in-order BST-তে sorted। BFS: level-order (queue)। BST গড়ে O(log n) কিন্তু skew হলে O(n); AVL rotation দিয়ে সবসময় O(log n)। B-Tree বহু-key, database/disk-এর জন্য। stack→DFS, queue→BFS।
