...
**💡 ধারণা**
Trie হলো এমন tree যেখানে প্রতিটা edge একটা অক্ষর, আর root থেকে কোনো node পর্যন্ত পথ = একটা prefix। একই prefix-এর শব্দগুলো পথ **শেয়ার** করে। ফলে prefix খোঁজা/autocomplete হয় শব্দের সংখ্যা নয়, বরং **শব্দের দৈর্ঘ্যের** সমান দ্রুত — O(L)।
...
**⏱️ Complexity** — build O(n); query/update O(log n)।
...
**🎯 কী শিখব**
- prefix sum + update, কম কোডে O(log n)
...
ft.update(2, 7);              // index 2-তে +7 (3→10)
console.log(ft.rangeSum(1, 3)); // 2+10+4 = 16
```

**🔍 ব্রেকডাউন**
- `i & -i` = সবচেয়ে ডানের set bit — এটাই কোন range কতটুকু ঢাকে ঠিক করে।
- update-এ bit যোগ করে উপরে, query-তে bit বিয়োগ করে নিচে যাই।
- range sum = দুই prefix sum-এর বিয়োগ।

**⏱️ Complexity** — update/query O(log n); space O(n)।

> **🎬 Animation Spec: Bit-Range Coverage**
> - **দৃশ্য:** array + BIT; প্রতিটা BIT index কোন range ঢাকে তা রঙিন bracket-এ।
...
দুটো অপ্টিমাইজেশন — **path compression** ও **union by rank/size** — এদের প্রায় O(1) (α, প্রায়-ধ্রুবক) করে। Kruskal MST (section 13), cycle detection, connectivity-তে অপরিহার্য।
...
**🔍 ব্রেকডাউন**
- **path compression:** find-এর সময় প্রতিটা node সরাসরি root-এ জোড়া → পরের find দ্রুত।
- **union by rank:** ছোট গাছকে বড় গাছের নিচে জোড়া → গাছ চ্যাপ্টা থাকে।
- `union` false দিলে দুই প্রান্ত আগেই এক গোষ্ঠীতে (Kruskal-এ cycle সংকেত)।

**⏱️ Complexity** — প্রতি অপারেশন প্রায় O(α(n)) ≈ O(1)।
...
**🔍 ব্রেকডাউন**
- সব suffix sort করলে একই prefix-ওয়ালা suffix পাশাপাশি আসে → binary search দিয়ে pattern খোঁজা যায়।
...
**⏱️ Complexity** — সরল build O(n² log n); খোঁজা O(m log n)। (উন্নত build O(n log n)।)
...
Trie = prefix tree, autocomplete O(L)। Segment Tree = range query+update O(log n)। Fenwick/BIT = হালকা prefix-sum O(log n)। Union-Find = dynamic grouping প্রায় O(1) (path compression + rank)। Suffix Array/Tree = string pattern matching-এর শক্তিশালী হাতিয়ার।

**পরবর্তী:** [15 — Complex & Disk-based Data Structures](15-complex-data-structures.md) →
